...
public class Node implements SortedFloatTree {
	...
	return 1 + (left.depth() < right.depth() ? right.depth() : left.depth());
    }
	...
	return (this.element == element) || (this.element < element ? right.exists(element) : left.exists(element));
    }

    @Override
    public SortedFloatTree insert(float element) {
	// TODO: Create a tree, with the inserted element in the right place.
	return new Node(this.element, (this.element < element?left:left.insert(element)), (this.element > element?right:right.insert(element)));
    }
}
